...
const Event: React.FC<{ event: ShiftEvent; resourceColor: string; currentView: 'week' | 'day' }> = ({
    event,
    resourceColor,
    currentView
}) => {
 ...
 */
const ResourceRow: React.FC<{ resource: Resource; daysInWeek: Date[]; currentView: 'week' | 'day' }> = ({
    resource,
    daysInWeek,
    currentView
}) => {
            ...
            <div className='flex flex-grow'>
                {daysInWeek.map((day) => (
                    <div
                        ...
                        style={{ height: '100px' }} // Fixed height for rows in both week and day views
                    >
                        {currentView === 'day' &&
 ...
 */
const MainCalendar: React.FC<{ currentWeekStart: Date; currentView: 'week' | 'day' | 'year' | 'quarter' }> = ({
    currentWeekStart,
    currentView
}) => {
    ...
    let columnsToDisplay: Date[] | { label: string; date: Date }[] = [];
    let headerTimeLabels: { label: string; key: string }[] = [];

    if (currentView === 'week') {
        columnsToDisplay = getDaysInWeek(currentWeekStart);
    } else if (currentView === 'day') {
        // For day view, columns are hours
        columnsToDisplay = [currentWeekStart]; // Still one "day" column, but its content is hourly
        headerTimeLabels = Array.from({ length: 24 }).map((_, i) => ({
    ...
    // Default rendering for 'week' and 'day' views
    return (
        <div className='flex flex-col overflow-hidden rounded-lg border border-gray-300 bg-white shadow-md'>
            ...
            <div className='flex border-b border-gray-200 bg-gray-100'>
                ...
                {/* Render hours for 'day' view, or dates for 'week' view */}
                {currentView === 'day'
                    ? headerTimeLabels.map((col) => (
                          <div
                              key={col.key}
            ...
            <div className='flex-grow overflow-y-auto' style={{ maxHeight: 'calc(100vh - 250px)' }}>
                <div className='flex'>
                    {/* No vertical time axis column here for any view */}
...
const App: React.FC = () => {
    ...
    const [currentView, setCurrentView] = useState<'week' | 'day' | 'year' | 'quarter'>('week');
