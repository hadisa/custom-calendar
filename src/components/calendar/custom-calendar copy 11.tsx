...
const Event: React.FC<{
    ...
    if (currentView === 'day') {
        ...
        return (
            <div
                className='relative mx-1 inline-block h-3 w-3 rounded-full' // Small circle indicator
                style={{
                    backgroundColor: resourceColor
                }}
                title={`${event.title} (${formatTime(event.start)} - ${formatTime(event.end)})`}></div>
        );
    } else {
        ...
        <div className='flex border-b border-gray-200 last:border-b-0'>
            ...
            <div className='flex flex-grow'>
                {daysInView.map((day) => (
                    <div
                        key={day.toISOString()}
                        className='relative flex-1 cursor-pointer border-r border-gray-200 last:border-r-0' // Added cursor-pointer
                ...
                <div className='mx-auto max-w-7xl py-6 sm:px-6 lg:px-8'>
                    ...
                    <div className='mb-6 flex items-center justify-between'>
                        ...
                        <Button onClick={() => openAddShiftForm()}>
                            {' '}
                            {/* Changed to call openAddShiftForm without date */}
                            Add New Shift
                        </Button>
                    </div>
                    <MainCalendar
